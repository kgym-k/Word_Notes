...
class NoteFragment() : Fragment() {
    ...
    override fun onCreateView(
        ...
        recyclerView = view.recyclerview_note
        ...
        mNoteViewModel.note.observe(viewLifecycleOwner, Observer { note ->
            ...
            // visibility
            val note_imageView: ImageView = view.findViewById(R.id.note_imageView)
            val note_text: TextView = view.findViewById(R.id.note_textView)
            if (adapter.itemCount == 0) {
                note_imageView.visibility = View.VISIBLE
                note_text.visibility = View.VISIBLE
            } else {
                note_imageView.visibility = View.INVISIBLE
                note_text.visibility = View.INVISIBLE
            }
        })

        // fab
        view.fab_note.setOnClickListener {
            val action =
    ...
    override fun onPrepareOptionsMenu(menu: Menu) {
        ...
        search.setOnActionExpandListener(object : MenuItem.OnActionExpandListener {
            override fun onMenuItemActionExpand(item: MenuItem?): Boolean {
                fab_note.visibility = View.GONE
                menu.getItem(1).setVisible(false)
                return true
            }

            override fun onMenuItemActionCollapse(item: MenuItem?): Boolean {
                fab_note.visibility = View.VISIBLE
    ...
    private fun sortBotomm() {
        ...
        val bottomSheetSortView = LayoutInflater.from(requireContext())
            .inflate(
                R.layout.layout_bottom_sheet_sort_note,
                requireView().findViewById(R.id.bottom_sheet_sort_container)
            )
    ...
    private fun deleteAllNote() {
        ...
        builder.setPositiveButton(getString(R.string.yes)) { _, _ ->
            ...
            Toast.makeText(requireContext(), getString(R.string.all_cards_are_removed), Toast.LENGTH_SHORT).show()
        }
        builder.setNegativeButton(getString(R.string.no)) { _, _ -> }
        builder.setTitle(getString(R.string.delete_all_cards_query))
        builder.setMessage(getString(R.string.all_cards_will_be_completely_deleted))
